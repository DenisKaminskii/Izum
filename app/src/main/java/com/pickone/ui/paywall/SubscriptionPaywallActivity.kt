...
import com.pickone.databinding.ActivitySubscriptionPaywallBinding
    ...
    private fun update(price: Price) = with(binding) {
        ...
        iPriceFirst.update(
            PremiumPriceItemState(
                ...
                titleColor = if (isWeekSelected) getColor(R.color.black_soft) else getColor(R.color.paywall_yellow),
                subtitleColor = if (isWeekSelected) getColor(R.color.grey_dark) else getColor(R.color.paywall_grey),
                icon = if (isWeekSelected) R.drawable.ic_paywall_lifetime_checked else R.drawable.ic_paywall_unchecked,
                accentColor = getColor(R.color.paywall_yellow)
            )
        )
